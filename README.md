...
3. Enter Play Mode.
4. Open the Control Room and independently start the session, conveyor, robot arms, and camera streams.

`Start Session` starts timing, counters, and CSV recording. It does not automatically start the conveyor, arms, or camera streams. `Stop Session` prevents new grasp tasks, waits for active arms to return to Idle, and then writes the session summary.

`MainSceneCopy2.unity` is enabled in the Build Settings, so a standalone build can use the same server while it is running on the same Mac.
